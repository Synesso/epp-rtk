...
/*
 * $Header: /cvsroot/epp-rtk/epp-rtk/java/src/com/tucows/oxrs/epp02/rtk/xml/EPPPoll.java,v 1.2 2004/01/19 18:47:59 tubadanm Exp $
 * $Revision: 1.2 $
...
import com.tucows.oxrs.epp02.rtk.xml.poll.*;
...
public class EPPPoll extends EPPXMLBase implements epp_Poll
{
     ...
     * look in the com.tucows.oxrs.epp02.rtk.xml.poll package for
     ...
     * So, let's say the first child of resData is "domain:transfer".
     * Non-alphanumeric characters are stripped out of the tag name and
     * the parser class "domaintransfer" is searched for first in
     * "com.tucows.oxrs.epp02.rtk.xml.poll".  It turns out that this class
     ...
     * If "domaintransfer" was not found in the default package, then
     ...
     * @see com.tucows.oxrs.epp02.rtk.RTKBase#getRTKProperties()
     */
        ...
        try
        {
            ...
            if ( response_data_element != null )
            {
                ...
                for (int count = 0; count < poll_resData_nodelist.getLength(); count++)
                {
                    ...
                    String poll_extension_package_name = "com.tucows.oxrs.epp02.rtk.xml.poll";
                    ...
                    while ( poll_res_data == null )
                    {
                        try
                        {
                            ...
                            poll_res_data = (PollResData) Class.forName(
                                            poll_extension_package_name + class_name ).newInstance();
                        }
