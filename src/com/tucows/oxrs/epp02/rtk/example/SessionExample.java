...
/*
 * $Header: /cvsroot/epp-rtk/epp-rtk/java/src/com/tucows/oxrs/epp02/rtk/example/SessionExample.java,v 1.1 2003/03/21 17:13:40 tubadanm Exp $
 * $Revision: 1.1 $
...
public class SessionExample
{
    ...
    private static String USAGE = "Usage: com.tucows.oxrs.epp02.rtk.example.SessionExample epp_host_name epp_host_port epp_client_id epp_password domain_name [contact_roid1] [contact_roid2]";
    ...
    public static void main(String args[])
    {
        ...
        try
        {
            ...
            String contact_roid1 = null;
            String contact_roid2 = null;
            if ( args.length > 5 )
            {
                contact_roid1 = args[5];
            }
            if ( args.length > 6 )
            {
                contact_roid2 = args[6];
            }
            ...
            try
            {
                     ...
                     ! EPPXMLBase.getCheckResultFor(check_results, domain_name).booleanValue() )
                {
                    ...
                    // Let's see if the user gave us a contact_roid to use
                    ...
                    if ( contact_roid1 != null )
                    {
                        ...
                        //
                        // ***************************
                        System.out.println("Creating the Contact Check command for ["+contact_roid1+"]");
                        ...
                        contact_list.add(contact_roid1);
                        if ( contact_roid2 != null )
                        {
                            contact_list.add(contact_roid2);
                        }
                        contact_check_request.setRoids( EPPXMLBase.convertListToStringArray(contact_list) );
                        ...
                        System.out.println("ContactCheck results: contact ["+contact_roid1+"] exists? ["+EPPXMLBase.getCheckResultFor(check_results, contact_roid1)+"]");
                        System.out.println("ContactCheck results: contact ["+contact_roid2+"] exists? ["+EPPXMLBase.getCheckResultFor(check_results, contact_roid2)+"]");
                        if ( EPPXMLBase.getCheckResultFor(check_results, contact_roid1) != null )
                        {
                            contact1_exists = EPPXMLBase.getCheckResultFor(check_results, contact_roid1).booleanValue();
                        }
                        if ( contact_roid2 != null &&
                             EPPXMLBase.getCheckResultFor(check_results, contact_roid2) != null )
                        {
                            contact2_exists = EPPXMLBase.getCheckResultFor(check_results, contact_roid2).booleanValue();
                        }
                    }

                    // ROID 1 will be used as the registrant for the domain.
                    if ( contact_roid1 == null || ! contact1_exists )
                    {
                        // ***************************
                        // Contact Create
                        //
                        // The given contact_roid1 does not exist in the registry
                        // or there was no contact_roid1 specified, so let's
                        // create one now.
                        //
                        // ***************************
                        ...
                        System.out.print("Dear registrant, please enter a passphrase for the new registrant contact: ");
                        ...
                        System.out.println("ContactCreate results: contact roid ["+contact_create_response.getRoid()+"]");
                        // save the resulting ROID so we can use this new contact
                        // in Domain operations.
                        contact_roid1 = contact_create_response.getRoid();

                    }

                    // ROID 2 will be used as the "tech" contact for the domain
                    // we'll be creating later.
                    if ( contact_roid2 == null || ! contact2_exists )
                    {
                        // ***************************
                        // Contact Create
                        //
                        // The given contact_roid2 does not exist in the registry
                        // or there was no contact_roid2 specified, so let's
                        // create one now.
                        //
                        // ***************************
                        System.out.println("Creating the Contact Create command");
                        ...
                        System.out.print("Dear registrant, please enter a passphrase for the new tech contact: ");
                        ...
                        System.out.println("ContactCreate results: contact roid ["+contact_create_response.getRoid()+"]");
                        // save the resulting ROID so we can use this new contact
                        // as the tech contact in Domain operations.
                        contact_roid2 = contact_create_response.getRoid();
                    ...
                    domain_create_request.setRegistrant( contact_roid1 );
                    List domain_contacts = new ArrayList();
                    // Use ROID 2 as the admin, billing and tech contact
                    // since some registries require at least of one
                    // each of these contact types.
                    domain_contacts.add( new epp_DomainContact( epp_DomainContactType.TECH, contact_roid2 ) );
                    domain_contacts.add( new epp_DomainContact( epp_DomainContactType.ADMIN, contact_roid2 ) );
                    domain_contacts.add( new epp_DomainContact( epp_DomainContactType.BILLING, contact_roid2 ) );
                ...
                // the auth ID if we're the owner.
                //
                // ***************************
                ...
                if ( domain_info_response.getAuthInfo() == null )
                {
                    ...
                    System.out.print("Dear registrant, please enter the passphrase for the domain you wish to transfer: ");
                    ...
                    System.out.println("DomainTransfer Results: transfer status ["+EPPXMLBase.transferStatusToString( domain_transfer_response.getTransferStatus() )+"]");
                ...
                else
                {
                    ...
                    {
                        if ( domain_nameservers[index].equals("ns1."+domain_name) )
                        {
                            System.out.println("removing ns1 from domain");
                            remove_list.add("ns1."+domain_name);
                            found_ns1 = true;
                        }
                        else if ( domain_nameservers[index].equals("ns2."+domain_name) )
                        {
                ...
                contact_info_request.setRoid( domain_info_response.m_registrant );
                ...
                try
                {
                    ...
                    if ( domain_transfer_response.getTransferStatus() == epp_TransferStatusType.PENDING )
                    {
