...
/*
 * $Header: /cvsroot/epp-rtk/epp-rtk/java/src/com/tucows/oxrs/epp0604/rtk/xml/EPPContactInfo.java,v 1.2 2006/04/24 21:20:25 ewang2004 Exp $
 * $Revision: 1.2 $
 ...
 * @see org.openrtk.idl.epp0604.contact.epp_ContactInfo
...
public class EPPContactInfo extends EPPContactBase implements epp_ContactInfo
{
     ...
     * @param value org.openrtk.idl.epp0604.epp_ContactInfoReq
     */
     ...
     * @return value org.openrtk.idl.epp0604.epp_ContactInfoRsp
     */
        ...
        try
        {
            ...
            for (int count = 0; count < contact_info_result_list.getLength(); count++)
            {
                ...
                if ( a_node.getNodeName().equals("contact:postalInfo") )
                {
                    ...
                    if ( action_response_.m_addresses == null )
                    {
                        action_response_.m_addresses = new epp_ContactNameAddress[1];
                        action_response_.m_addresses[0] = epp_address;
                    }
                    else
                    {
                        // turn the fixed-size array into a list so we can easily add another element
                        // to it, then convert it back to an array -- I love being lazy!
                        List addresses = java.util.Arrays.asList(action_response_.m_addresses);
                        addresses.add(epp_address);
                        action_response_.m_addresses = (epp_ContactNameAddress[]) EPPXMLBase.convertListToArray((new epp_ContactNameAddress()).getClass(), addresses);
                    }
                    
                    // XXX Backward compatibility for deprecated data members
                    if ( action_response_.m_ascii_address == null )
                    {
